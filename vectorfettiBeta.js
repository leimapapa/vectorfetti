function confetti(el, params) {
	...
	for (let i = 0; i < c.flakes; i++) {
		...
		if (c.colors == "random" || !Array.isArray(c.colors)) {
			confettiColor = `rgb(${randInt(0, 255)}, ${randInt(0, 255)}, ${randInt(
				0,
				255
			)})`;
		} else {
		...
		if (!Array.isArray(c.type)) {
				...
				case "xo":
					if (randInt(0, 1) == 1) {
						...
						confettiType = `<circle id="${conId}" stroke="${confettiColor}" stroke-width="${randInt(
							1,
							3
						)}" fill="none" cx="0" cy="0" r="${fixedR}" filter="url(#blur${randomBlur()})">
			${confettiSpin}
			</circle>`;
					} else {
						// X shape
			...
			${confettiSpin}
					...
					break;
				case "code":
					confettiType = `<text id="${conId}" fill="${confettiColor}" fill="none" x="0" y="0" dominant-baseline="middle" text-anchor="middle" filter="url(#blur${randomBlur()})" font-size="${fixedFontSize}" transform="rotate(${
						c.angleEmoji
					})">${randInt(0, 1)}
			${confettiSpin}
			</text>`;
				...
				case "fruit":
					...
					confettiType = `<text id="${conId}" fill="${confettiColor}" stroke-width="${randInt(
						1,
						3
					)}" fill="none" x="0" y="0" dominant-baseline="middle" text-anchor="middle" filter="url(#blur${randomBlur()})" font-size="${fixedFontSize}" transform="rotate(${
						c.angleEmoji
					})">${fruitArray[randInt(0, fruitArray.length - 1)]}
			${confettiSpin}
			</text>`;
					break;
				case "circle":
			...
			${confettiSpin}
					...
					break;
				case "flower":
					confettiType = `<g filter="url(#blur${randomBlur()})" id="${conId}" transform="scale(${
						c.scale
					})">
				...
				<circle fill="rgb(${randInt(0, 255)}, ${randInt(0, 255)}, ${randInt(
						0,
						255
					)})" cx="0" cy="0" r="2.8" stroke="rgb(${randInt(0, 255)}, ${randInt(
						0,
						255
					)}, ${randInt(0, 255)})" stroke-width="0.4" />
				${confettiSpin}
				</g>`;
					break;
				case "square":
				...
				case "cards":
					let cardType = ["♣", "♠", "♦", "♥"];
					let cardTypePath = [
						"M 0 1 C 2 3 3 -1 0.75 -0.25 C 1.5 -2 0 -2 0 -2 C 0 -2 -1.5 -2 -0.75 -0.25 C -3 -1 -2 3 0 1 L -1 2 L 1 2 Z",
						"M 0 1 C 2 2 3 0 0 -2 C -3 0 -2 2 0 1 L -1 2 L 1 2 Z",
						"M 0 2 L 1.5 0 L 0 -2 L -1.5 0 Z",
						"M 0 2 C 4 -1 1 -3 0 -1 C -1 -3 -4 -1 0 2"
					];
					...
					<path d="${cardTypePath[randCard]}" fill="${
						cardCol[randCard]
					}" transform="scale(0.5 0.5) translate(-7 -9)" />
					<path d="${cardTypePath[randCard]}" fill="${
						cardCol[randCard]
					}" transform="scale(0.5 0.5) translate(7 9)" />
					</g>
				...
				case "image":
					confettiType = `<g transform="scale(${c.scale})" id="${conId}">
						<image href="https://assets.codepen.io/2045658/rick.gif" height="${
							fixedScale * 20
						}" width="${fixedScale * 20}" x="${fixedScale * -10}" y="${
						fixedScale * -10
					}">
							${confettiSpin}
						</image>
				...
				case "default":
				default:
					confettiType = `<rect id="${conId}" fill="${confettiColor}" x="0" y="0" height="${randInt(
						6,
						18
					)}" width="${randInt(4, 7)}" filter="url(#blur${randomBlur()})">
						${confettiSpin}
					</rect>`;
			}
		} else {
			...
			if (isImage(randArrVal)) {
				confettiType = `<g transform="scale(${c.scale})" id="${conId}">
						<image href="${randArrVal}" height="${fixedScale * 20}" width="${
					fixedScale * 20

...
function confetti(el = document.body, opt_properties) {
  ...
  for (let i = 0; i < c.flakes; i++) {
    ...
    let confettiType = ``;
    if (c.spin) {
      confettiSpin = `<animateTransform
      attributeName="transform" type="rotate"
      values="0 0 0; ${(Math.random() < 0.5 ? -1 : 1) * 360} 0 0"
      dur="${randInt(c.speed / 6, c.speed / 2)}ms"
      begin="-${randInt(100, 5000)}ms"
    ...
    confettiSpin += `<animateTransform
      ...
      values="1 1; ${xySpin}; 1 1"
      dur="${randInt(c.speed / 16, c.speed / 10)}ms"
      repeatCount="indefinite" additive="sum"
    />`;
    }
    ...
    if (c.colors == 'random' || !Array.isArray(c.colors)) {
      confettiColor = `hsl(${randInt(0, 255)}, ${randInt(50, 100)}%, ${randInt(25, 75)}%)`;
    } else {
    ...
    if (!Array.isArray(c.type)) {
      ...
      switch (c.type) {
        ...
        case 'square':
          ...
          confettiType = `<rect id="${conId}" fill="${confettiColor}" x="${-hhww/2}" y="${-hhww/2}" height="${hhww}" width="${hhww}" filter="url(#blur${randomBlur()})">
                ${confettiSpin}
            </rect>`;
          break;
        case 'tri':
        case 'triangle':
          confettiType = `<path id="${conId}" fill="${confettiColor}" d="M -8 6 L 0 -8 L 8 6 Z" filter="url(#blur${randomBlur()})" transform="rotate(${randInt(0,360)} 0 0)">
                ${confettiSpin}
            </path>`;
          break;
        case 'rect':
        case 'default':
        default: {
					let hhh = randInt(6, 18);
					let www = randInt(4, 7);
          confettiType = `<rect id="${conId}" fill="${confettiColor}" x="${-www/randInt(2,www)}" y="${-hhh/randInt(2,hhh)}" height="${hhh}" width="${www}" filter="url(#blur${randomBlur()})">
            ${confettiSpin}
        </rect>`;
				}
      }
    ...
    animatedConfetti += `<g>
      ${confettiType}
			<animateMotion xlink:href="#${conId}" dur="${confettiDur}ms" begin="0s" fill="freeze" repeatCount="none" keyTimes="0;1" keySplines="${randInt(0,1)/10} 0.4 ${randInt(0,10)/10} ${randInt(4,5)/10}" calcMode="spline" 
			path="M ${ww * 0.5} ${hh * 0.5} 
			Q ${ww * randInt(25,75)/100} ${topY} ${ww * randInt(25,75)/100} ${topY} 
			T ${ww * randInt(25,75)/100} ${hh * randInt(40,60)/100} 
			T ${ww * randInt(0,100)/100} ${hh}">
			</animateMotion>
		</g>`;
  }
  ...
  const centerX = elemRect.left - (elemRect.left - elemRect.right) / 2;
	...
	<aanimateTransform attributeName="transform" type="translate" additive="sum" values="0 0; ${-ww / 2} ${-hh / 2}" begin="0s" dur="${c.speed / 10}ms" fill="freeze" repeatCount="0" keyTimes="0;1" keySplines="0.4 0 0.6 1" calcMode="spline" />
	<aanimateTransform attributeName="transform" type="scale" additive="sum" values="0; 1" begin="0s" dur="${c.speed / 10}ms" fill="freeze" repeatCount="0" keyTimes="0;1" keySplines="0.4 0 0.6 1" calcMode="spline" />`;
	}
